...
jQuery(function($) {
	...
	// Upload image
	$('.cuztom_td').on( 'click', '.cuztom_upload_old', function() 
	{
		that		= $(this);
		...
		window.send_to_editor = function( html ) {
			...
			// Add remove button
			that.after('<a href="#" class="cuztom_remove_' + type + '">' + ( type == 'image' ? Cuztom.remove_image : Cuztom.remove_file ) + '</a> ')
		}
	    
		return false;
	});
  $('.cuztom_td').on( 'click', '.cuztom_upload_new', function() 
	{
	    ...
	    formfield 	= parent.find('.cuztom_hidden').attr('name');
    		...
    		wp.media.editor.send.attachment = function(props, attachment){
					...
					if( type == 'image' )
					{
						...
						// Add the image to the preview
						html 	= attachment.url;
						spanID.html('<img src="'+ html + '" width="300" />' );
					}
					else
					{
						url		= attachment.url;
						...
						anchor	=  attachment.url.substring(attachment.url.lastIndexOf('/')+1);;
						html	= $('<span class="cuztom_mime"><a href="' + url + '" target="_blank">' + anchor + '</a></span>' );
						spanID.html( html );
					}
